...
display average ratings before participants rate.  
This can bias ratings and is called Social Influence Bias (SIB); 
the tendency for individuals to conform to what they perceive as the 
``norm" in a community.
...
we build a baseline dataset with an initial set of participants
by allowing them to rate twice: before seeing the average rating,
and after seeing it.  
...
As a case study, we evaluate our approach on a dataset of 9390 ratings from the California Report
Card (CRC), a new recommender system that encourages political
engagement.
We found statistically significant evidence of SIB, and mitigating models 
were able to predict changed ratings with a normalized RMSE of 12.8\% and 
reduce bias by 76.3\%.
...
\fontsize{7.3pt}{7.5pt} \selectfont
